...
from det3d.models.utils.dsvt_utils import get_window_coors, get_inner_win_inds_cuda, get_pooling_index, get_continous_inds
from det3d.models.utils.dsvt_utils import PositionEmbeddingLearned
from det3d.models.utils.tensorrt_utils.trtwrapper import TRTWrapper
...
class DSVT(nn.Module):
    ...
    def __init__(self, model_cfg, **kwargs):
        ...
        self.reduction_type = getattr(self.model_cfg, "reduction_type", "attention")
        # save GPU memory
        self.use_torch_ckpt = getattr(self.model_cfg, "ues_checkpoint", False)
...
class SetAttention(nn.Module):
    ...
    def forward(self, src, pos=None, key_padding_mask=None, voxel_inds=None, onnx_export=False):
        ...
        '''
        set_features = src[voxel_inds]  # [118594, 5], [5051, 48]
        if pos is not None:
            set_pos = pos[voxel_inds]   # [118594, 192], [5051, 48]
        else:
            set_pos = None
        if pos is not None:
            query = set_features + set_pos # [5051, 48, 5] + [5051, 48, 192]
...
class DSVT_TrtEngine(nn.Module):
    def __init__(self, model_cfg, **kwargs):
        ...
        dropout = self.model_cfg.dropout
        activation = self.model_cfg.activation
        self.reduction_type = getattr(self.model_cfg, "reduction_type", "attention") # check your cfg
        stage_num = len(block_name)
