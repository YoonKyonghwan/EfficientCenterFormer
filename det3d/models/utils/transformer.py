...
class PoolFormerBlock(nn.Module):
    def __init__(self, dim, pool_size=3, mlp_dim=256, 
        ...
        self.mlp = FeedForward(dim, mlp_dim, dropout=drop)
...
class Poolformer(nn.Module):
    def __init__(
        ...
        for _ in range(depth):
            self.layers.append(PoolFormerBlock(dim, mlp_dim=mlp_dim, drop=dropout))

    def forward(self, x, center_pos_embedding):
        for idx, poolformer_block in enumerate(self.layers):
            if center_pos_embedding is not None:
                x = x + center_pos_embedding
                x = poolformer_block(x)
            else:
                x = poolformer_block(x)
        return x
    
    # def forward(
    #     self, x, pos_embedding, center_pos
    # ):
    #     if pos_embedding is not None:
    #         center_pos_embedding = pos_embedding(center_pos)
    #     # reference_points = center_pos[:, :, None, :2].repeat(1, 1, self.levels, 1)
