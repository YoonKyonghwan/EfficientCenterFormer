...
from ..utils.transfusion_utils import clip_sigmoid
from ..utils import centernet_utils
from ..utils.basic_block_2d import BasicBlock2D
from ..utils.transfusion_utils import PositionEmbeddingLearned, TransformerDecoderLayer
            ...
            for k in range(num_conv - 1):
                fc_list.append(nn.Sequential(
                    nn.Conv1d(input_channels, head_channels, kernel_size,
                              stride=1, padding=kernel_size//2, bias=use_bias),
                    nn.BatchNorm1d(head_channels),
                    nn.ReLU()
                ))
            fc_list.append(nn.Conv1d(head_channels, output_channels,
                           kernel_size, stride=1, padding=kernel_size//2, bias=True))
...
class TransFusionHead(nn.Module):
    ...
    def __init__(
        ...
        self.feature_map_stride = getattr(
            self.model_cfg.TARGET_ASSIGNER_CONFIG, 'FEATURE_MAP_STRIDE', None)
        self.dataset_name = getattr(
            self.model_cfg.TARGET_ASSIGNER_CONFIG, 'DATASET', 'nuScenes')
        ...
        self.query_range = torch.arange(-self.query_radius,
                                        self.query_radius+1)
        self.query_r_coor_x, self.query_r_coor_y = torch.meshgrid(
            self.query_range, self.query_range)

        num_heads = self.model_cfg.NUM_HEADS
        dropout = self.model_cfg.DROPOUT
        activation = self.model_cfg.ACTIVATION
        ffn_channel = self.model_cfg.FFN_CHANNEL
        bias = getattr(self.model_cfg, 'USE_BIAS_BEFORE_NORM', False)

        loss_cls = self.model_cfg.LOSS_CONFIG.LOSS_CLS
        self.use_sigmoid_cls = getattr(loss_cls, "use_sigmoid", False)
        if not self.use_sigmoid_cls:
            self.num_classes += 1
        self.loss_cls = loss_utils.SigmoidFocalClassificationLoss(
            gamma=loss_cls.gamma, alpha=loss_cls.alpha)
        ...
        self.loss_iou_rescore_weight = self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS[
            'loss_iou_rescore_weight']

        self.code_size = 10

        # a shared convolution
        self.shared_conv = nn.Conv2d(
            in_channels=input_channels, out_channels=hidden_channel, kernel_size=3, padding=1)
        layers = []
        layers.append(BasicBlock2D(hidden_channel, hidden_channel,
        ...
        heads['heatmap'] = dict(
            out_channels=self.num_classes, num_conv=self.model_cfg.NUM_HM_CONV)
        self.prediction_head = SeparateHead_Transfusion(
            hidden_channel, 64, 1, heads, use_bias=True)

        self.init_weights()
        self.bbox_assigner = HungarianAssigner3D(
            **self.model_cfg.TARGET_ASSIGNER_CONFIG.HUNGARIAN_ASSIGNER)
    ...
    def predict(self, inputs):
        ...
        top_proposals_key_x = top_proposals_x[:, :, None, None] + \
            self.query_r_coor_x[None, None, :, :].to(top_proposals.device)
        top_proposals_key_y = top_proposals_y[:, :, None, None] + \
            self.query_r_coor_y[None, None, :, :].to(top_proposals.device)
        # bs, num_proposals, key_num
        top_proposals_key_index = top_proposals_key_x.view(
            batch_size, top_proposals_key_x.shape[1], -1) * x_grid + top_proposals_key_y.view(batch_size, top_proposals_key_y.shape[1], -1)
        key_mask = (top_proposals_key_index < 0) + \
            (top_proposals_key_index >= (x_grid * y_grid))
        top_proposals_key_index = torch.clamp(
            top_proposals_key_index, min=0, max=x_grid * y_grid-1)
        num_proposals = top_proposals_key_index.shape[1]
        key_feat = lidar_feat_flatten.gather(index=top_proposals_key_index.view(
            batch_size, 1, -1).expand(-1, lidar_feat_flatten.shape[1], -1), dim=-1)
        key_feat = key_feat.view(
            batch_size, lidar_feat_flatten.shape[1], num_proposals, -1)
        key_pos = bev_pos.gather(index=top_proposals_key_index.view(
            batch_size, 1, -1).permute(0, 2, 1).expand(-1, -1, bev_pos.shape[-1]), dim=1)
        key_pos = key_pos.view(
            batch_size, num_proposals, -1, bev_pos.shape[-1])
        key_feat = key_feat.permute(0, 2, 1, 3).reshape(
            batch_size*num_proposals, lidar_feat_flatten.shape[1], -1)
        key_pos = key_pos.view(-1, key_pos.shape[2], key_pos.shape[-1])
        key_padding_mask = key_mask.view(-1, key_mask.shape[-1])

        query_feat_T = query_feat.permute(0, 2, 1).reshape(
        ...
        query_feat = query_feat_T.reshape(
            batch_size, num_proposals, 128).permute(0, 2, 1)
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dict):
        ...
        bbox_targets = torch.zeros(
            [num_proposals, self.code_size]).to(center.device)
        ...
        for idx in range(len(gt_bboxes_3d)):
            ...
            if width > 0 and length > 0:
                radius = centernet_utils.gaussian_radius(
                    length.view(-1), width.view(-1), target_assigner_cfg.GAUSSIAN_OVERLAP)[0]
                radius = max(target_assigner_cfg.MIN_RADIUS, int(radius))
                x, y = gt_bboxes_3d[idx][0], gt_bboxes_3d[idx][1]

                coor_x = (
                    x - self.point_cloud_range[0]) / self.voxel_size[0] / self.feature_map_stride
                coor_y = (
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, pred_dicts, **kwargs):
        ...
        loss_dict["loss_heatmap"] = loss_heatmap.item() * \
            self.loss_heatmap_weight
        loss_all += loss_heatmap * self.loss_heatmap_weight

        labels = labels.reshape(-1)
        label_weights = label_weights.reshape(-1)
        cls_score = pred_dicts["heatmap"].permute(
            0, 2, 1).reshape(-1, self.num_classes)

        one_hot_targets = torch.zeros(
            *list(labels.shape), self.num_classes+1, dtype=cls_score.dtype, device=labels.device)
        ...
        preds = torch.cat([pred_dicts[head_name]
                          for head_name in self.model_cfg.SEPARATE_HEAD_CFG.HEAD_ORDER], dim=1).permute(0, 2, 1)
        ...
        loss_all = loss_all + loss_cls * self.loss_cls_weight + \
            loss_bbox * self.loss_bbox_weight
        ...
        if "iou" in pred_dicts.keys():
            ...
            center_iou[:, 0, :] = center_iou[:, 0, :] * self.feature_map_stride * \
                self.voxel_size[0] + self.point_cloud_range[0]
            center_iou[:, 1, :] = center_iou[:, 1, :] * self.feature_map_stride * \
                self.voxel_size[1] + self.point_cloud_range[1]
            batch_box_targets_for_iou = torch.cat(
                [center_iou, height_iou, dim_iou, rot_iou], dim=1).permute(0, 2, 1)
            ...
            center_pred[:, 0, :] = center_pred[:, 0, :] * self.feature_map_stride * \
                self.voxel_size[0] + self.point_cloud_range[0]
            center_pred[:, 1, :] = center_pred[:, 1, :] * self.feature_map_stride * \
                self.voxel_size[1] + self.point_cloud_range[1]
            batch_box_preds = torch.cat(
                [center_pred, height_pred, dim_pred, rot_pred], dim=1).permute(0, 2, 1)
            ...
            loss_dict[f"loss_iou"] = layer_iou_loss.item() * \
                self.loss_iou_rescore_weight
    ...
    def decode_bbox(self, heatmap, rot, dim, center, height, vel, filter=False):
        ...
        center[:, 0, :] = center[:, 0, :] * self.feature_map_stride * \
            self.voxel_size[0] + self.point_cloud_range[0]
        center[:, 1, :] = center[:, 1, :] * self.feature_map_stride * \
            self.voxel_size[1] + self.point_cloud_range[1]
        dim = dim.exp()
    ...
    def get_bboxes(self, preds_dicts):
        ...
        batch_score = batch_score * \
            preds_dicts["query_heatmap_score"] * one_hot
        ...
        batch_iou = (preds_dicts['iou'] + 1) * \
            0.5 if 'iou' in preds_dicts else None
        ...
        if self.dataset_name == "nuScenes":
            self.tasks = [
                dict(num_class=8, class_names=[], indices=[
                     0, 1, 2, 3, 4, 5, 6, 7], radius=-1),
                dict(num_class=1, class_names=[
                     "pedestrian"], indices=[8], radius=0.175),
                dict(num_class=1, class_names=[
                     "traffic_cone"], indices=[9], radius=0.175),
            ]
        elif self.dataset_name == "Waymo":
            self.tasks = [
                dict(num_class=1, class_names=[
                     "Car"], indices=[0], radius=0.7),
                dict(num_class=1, class_names=[
                     "Pedestrian"], indices=[1], radius=0.7),
                dict(num_class=1, class_names=[
                     "Cyclist"], indices=[2], radius=0.7),
            ]
        ...
        for i in range(batch_size):
            ...
            # IOU refine
            if getattr(self.model_cfg.POST_PROCESSING, 'USE_IOU_TO_RECTIFY_SCORE', False) and batch_iou is not None:
                pred_iou = torch.clamp(batch_iou[i][0][cmask], min=0, max=1.0)
                IOU_RECTIFIER = scores.new_tensor(
                    self.model_cfg.POST_PROCESSING.IOU_RECTIFIER)
                if len(IOU_RECTIFIER) == 1:
                    IOU_RECTIFIER = IOU_RECTIFIER.repeat(self.num_classes)
                scores = torch.pow(
                    scores, 1 - IOU_RECTIFIER[labels]) * torch.pow(pred_iou, IOU_RECTIFIER[labels])
            ...
            for task in self.tasks:
                ...
                if task["radius"] > 0:
                    ...
                    task_nms_config = copy.deepcopy(
                        self.model_cfg.POST_PROCESSING.NMS_CONFIG)
                ...
                if task_keep_indices.shape[0] != 0:
                    keep_indices = torch.where(task_mask != 0)[
                        0][task_keep_indices]
                    keep_mask[keep_indices] = 1
            keep_mask = keep_mask.bool()
            ret = dict(
                pred_boxes=boxes3d[keep_mask], pred_scores=scores[keep_mask], pred_labels=labels[keep_mask])
